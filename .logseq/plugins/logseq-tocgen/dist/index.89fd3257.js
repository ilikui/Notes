...
function TocGen({ slot: slot1 , uuid , root: root1 , height , levels , headingType: headingType1 , blocksToHighlight , pushRoot , removeRoots ,  }) {
...
async function main() {
  ...
  `);
    ...
    logseq.App.registerPageMenuItem(t$2("Open TOC"), async ({ page  })=>openPageTOC(page));
...
async function openPageTOC(pageName) {
    ...
    await logseq.Editor.appendBlockInPage("contents", contentsEl?.clientHeight + 100 < parent.window.innerHeight ? `{{renderer :tocgen2, [[${pageName}]], calc(100vh - ${contentsEl.clientHeight + 100}px)}}` : `{{renderer :tocgen2, [[${pageName}]]}}`);
